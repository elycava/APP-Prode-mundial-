...
[25,"E","Domingo 14 de Junio","14:00","Alemania","Curazao","de","cw",null,null],
...
const matchesData = rows.map(r => ({
  id:r[0], group:r[1], date:r[2], time:r[3] + " hs",
  home:r[4], away:r[5],
  homeFlag:flag(r[6]), awayFlag:flag(r[7]),
  realHome:r[8], realAway:r[9]
}));
...
const knockoutStages = Object.fromEntries(Object.entries(ko).map(([k,v]) => [
  k,
  v.map(x => ({id:x[0],label:x[1],date:x[2],time:x[3]+" hs",home:x[4],away:x[5],realHome:null,realAway:null}))
]));
...
function showSection(id){
  ...
  [...document.querySelectorAll(".menu button")].find(b => b.getAttribute("onclick").includes(id))?.classList.add("active");
...
function card(m){
  ...
  <div class="match-card">
    ...
    <div class="match-teams">
      <div class="team">${m.homeFlag ? `<img src="${m.homeFlag}" class="flag">` : ""}<span>${m.home}</span></div>
      ...
      <div class="team">${m.awayFlag ? `<img src="${m.awayFlag}" class="flag">` : ""}<span>${m.away}</span></div>
    </div>
...
function calculateScore(){
  ...
  all.forEach(m => {
    ...
    const a = parseInt(ph), b = parseInt(pa);
    if(result(a,b) === result(m.realHome,m.realAway)){ total++; outcome++; }
    if(a === m.realHome && b === m.realAway){ total++; exact++; }
  });
